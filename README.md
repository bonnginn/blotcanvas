...
- 画像本体を含まない軽量プロジェクトJSONと、再利用可能な設定テンプレートを保存
...
画像の読み込み、調整、切り出し、Figure作成はブラウザ内で実行します。BlotCanvasが元画像をサーバーへアップロードまたは保存する機能はありません。プロジェクトJSONには画像本体を含めず、ファイル名・相対パス・処理設定を保存します。再開時はJSONを開いた後、元画像を含むフォルダを選択して再リンクしてください。ブラウザの安全制限により、JSONと同じフォルダに画像があっても、利用者によるフォルダ選択が必要です。
