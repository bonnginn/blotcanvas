...
- TIFF・PNG・JPEG画像に対応
- 撮影ごとの水平補正、切り出し領域、CLの階調・明るさ・コントラストを調整
- 同じゲル由来のブロットで横幅を統一
- Bio-Rad Precision Plus Protein All Blueを含むマーカープリセットと自作マーカー登録
- レーン条件を階層・反復・±条件として編集
- Illustratorで編集可能なFigure SVGと、切り出し位置を示した証拠保全SVGを出力
...
4. CL画像の階調・明るさ・コントラストを調整します。
5. 使用する分子量と、画像上のマーカーバンド位置を指定します。
6. レーン数と実験条件を入力します。
7. Figureの順番を確認し、SVG・JSONを保存します。
...
BlotCanvas本体は[MIT License](./LICENSE)で公開しています。著作権者名は `bonnginn` としています。利用しているオープンソースソフトウェアについては[Third-Party Notices](./THIRD_PARTY_NOTICES.md)を参照してください。
